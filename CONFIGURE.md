...
| --- | --- | --- |
| Device ID | This field will contain the device ID of the Azure IoT device as it is/will be on IoT Hub or IoT Central. If the Connection Type == Device provisioning service, and the enrollment is an individual Enrollment, enter the "Registration id" for that enrollment here.  | - |
...
| SAS Key | The [symmetric key](https://docs.microsoft.com/en-us/azure/iot-dps/concepts-symmetric-key-attestation) to authenticate with the Device Provisioning Service (DPS) instance or Azure IOT Central. If Connection Type = Connection string, this is the SAS in the device properties. If Connection Type = DPS, this is the SAS found in the enrollment properties. | Authentication Method |
